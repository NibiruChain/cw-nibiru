use super::{
    intrinsics::{
        tbaa_label, type_to_llvm, CtxType, FunctionCache, GlobalCache,
...
use inkwell::{
    ...
    types::{
        BasicType, BasicTypeEnum, FloatMathType, IntType, PointerType,
        VectorType,
    },
    values::{
        BasicMetadataValueEnum, BasicValue, BasicValueEnum, FloatValue,
        FunctionValue, InstructionOpcode, InstructionValue, IntValue, PhiValue,
        PointerValue, VectorValue,
    },
    AddressSpace, AtomicOrdering, AtomicRMWBinOp, DLLStorageClass,
...
use wasmer_types::{
    ...
    MemoryIndex, ModuleInfo, SignatureIndex, TableIndex, Type,
};
...
impl FuncTranslator {
    ...
    pub fn translate_to_module(
        ...
        let module_name = match wasm_module.name.as_ref() {
            ...
            Some(module_name) => {
                format!("module {} function {}", module_name, function_name)
            }
        };
        ...
        let phis: SmallVec<[PhiValue; 1]> = wasm_fn_type
            ...
            .iter()
            .map(|&wasm_ty| {
                type_to_llvm(&intrinsics, wasm_ty)
                    .map(|ty| builder.build_phi(ty, "").unwrap())
            })
            .collect::<Result<_, _>>()?;
        ...
        let mut params = vec![];
        let first_param = if func_type.get_return_type().is_none()
            && wasm_fn_type.results().len() > 1
        {
            2
        } else {
            1
...
impl<'ctx, 'a> LLVMFunctionCodeGenerator<'ctx, 'a> {
    ...
    fn trunc_sat<T: FloatMathType<'ctx>>(
        ...
        let value = err!(self.builder.build_bit_cast(value, fvec_ty, ""))
            .into_vector_value();
        let zero = fvec_ty.const_zero();
        let lower_bound =
            self.splat_vector(lower_bound.as_basic_value_enum(), fvec_ty)?;
        let upper_bound =
            self.splat_vector(upper_bound.as_basic_value_enum(), fvec_ty)?;
        let nan_cmp = err!(self.builder.build_float_compare(
            FloatPredicate::UNO,
            value,
            zero,
            "nan"
        ...
        let value = err!(self.builder.build_select(
            not_representable,
            zero,
            value,
            "safe_to_convert"
    ...
    fn v128_into_int_vec(
        ...
        Ok((
            err!(self.builder.build_bit_cast(value, int_vec_ty, ""))
                .into_vector_value(),
            info,
        ))
    }
    ...
    fn apply_pending_canonicalization(
        ...
        if info.has_pending_f32_nan() {
            if value.get_type().is_vector_type()
                || value.get_type()
                    == self.intrinsics.i128_ty.as_basic_type_enum()
            {
    ...
    fn canonicalize_nans(
        ...
        if f_ty.is_vector_type() {
            ...
            let canonical_qnan = f_ty
                ...
                .const_float(f64::NAN);
            let canonical_qnan =
                self.splat_vector(canonical_qnan.as_basic_value_enum(), f_ty)?;
            err_nt!(self
                .builder
    ...
    fn resolve_memory_ptr(
        ...
        let var_offset =
            err!(builder.build_int_z_extend(var_offset, intrinsics.i64_ty, ""));
        let offset = err!(builder.build_int_add(var_offset, imm_offset, ""));

        // Look up the memory base (as pointer) and bounds (as unsigned integer).
        let base_ptr = match self.ctx.memory(
            memory_index,
            intrinsics,
            self.module,
            self.memory_styles,
            ...
            MemoryCache::Dynamic {
                ...
                let value_size_v =
                    intrinsics.i64_ty.const_int(value_size as u64, false);
                let ptr_in_bounds = if offset.is_const() {
                    ...
                    let ptr_in_bounds = load_offset_end.const_int_compare(
                        ...
                        intrinsics
                            .i64_ty
                            .const_int(minimum.bytes().0 as u64, false),
                    );
                    ...
                    || ptr_in_bounds.get_zero_extended_constant().unwrap() != 1
                {
                    ...
                    let in_bounds_continue_block = context.append_basic_block(
                        *function,
                        "in_bounds_continue_block",
                    );
                    let not_in_bounds_block = context
                        .append_basic_block(*function, "not_in_bounds_block");
                ...
                let ptr_to_base = err!(builder.build_load(
                    intrinsics.ptr_ty,
                    ptr_to_base_ptr,
                    ""
                ))
    ...
    fn finalize(
        ...
        let results =
            self.state.popn_save_extra(wasm_fn_type.results().len())?;
        ...
        if wasm_fn_type.results().is_empty() {
            ...
            for (idx, value) in results.into_iter().enumerate() {
                ...
                struct_value = err!(self.builder.build_insert_value(
                    struct_value,
                    value,
                    idx as u32,
                    ""
            ...
            err!(self.builder.build_return(Some(
    ...
    fn translate_operator(
        ...
        match op {
            ...
            Operator::Block { blockty } => {
                ...
                let end_block =
                    self.context.append_basic_block(self.function, "end");
                ...
                let phis: SmallVec<[PhiValue<'ctx>; 1]> = self
                    ...
                    .map(|&wp_ty| {
                        err_nt!(wptype_to_type(wp_ty)).and_then(|wasm_ty| {
                            type_to_llvm(self.intrinsics, wasm_ty).and_then(
                                |ty| err_nt!(self.builder.build_phi(ty, "")),
                            )
                        })
                    })
                    .collect::<Result<_, _>>()?;
            ...
            Operator::Loop { blockty } => {
                let loop_body =
                    self.context.append_basic_block(self.function, "loop_body");
                let loop_next =
                    self.context.append_basic_block(self.function, "loop_outer");
                ...
                for phi in loop_phis.iter().rev() {
                    ...
                    let value =
                        self.apply_pending_canonicalization(value, info)?;
            ...
            Operator::BrTable { ref targets } => {
                ...
                let default_frame =
                    self.state.frame_at_depth(targets.default())?;
            ...
            Operator::If { blockty } => {
                ...
                let if_then_block =
                    self.context.append_basic_block(self.function, "if_then");
                let if_else_block =
                    self.context.append_basic_block(self.function, "if_else");
                let end_block =
                    self.context.append_basic_block(self.function, "if_end");
                ...
                let then_phis: SmallVec<[PhiValue<'ctx>; 1]> = block_param_types
                    ...
                    .map(|&ty| err_nt!(self.builder.build_phi(ty, "")))
                    .collect::<Result<SmallVec<_>, _>>()?;
                for (else_phi, then_phi) in
                    else_phis.iter().rev().zip(then_phis.iter().rev())
                {
                    let (value, info) = self.state.pop1_extra()?;
                    let value =
                        self.apply_pending_canonicalization(value, info)?;
            ...
            Operator::Else => {
                ...
                *if_else_state = IfElseState::Else;
                ...
                if let ControlFrame::IfElse { else_phis, .. } =
                    self.state.frame_at_depth(0)?
                {
            ...
            Operator::End => {
                ...
                {
                    for (phi, else_phi) in
                        frame.phis().iter().zip(else_phis.iter())
                    {
                        phi.add_incoming(&[(
                            &else_phi.as_basic_value(),
                            *if_else,
                        )]);
                    }
            ...
            Operator::F32Const { value } => {
                let bits =
                    self.intrinsics.i32_ty.const_int(value.bits() as u64, false);
                let info = if is_f32_arithmetic(value.bits()) {
                    ExtraInfo::arithmetic_f32()
                } else {
                    Default::default()
                };
                let f = err!(self.builder.build_bit_cast(
                    bits,
            ...
            Operator::I8x16Splat => {
                ...
                let v = err!(self.builder.build_int_truncate(
                    v,
                    self.intrinsics.i8_ty,
                    ""
                ));
                let res = self.splat_vector(
                    v.as_basic_value_enum(),
                    self.intrinsics.i8x16_ty,
                )?;
                let res = err!(self.builder.build_bit_cast(
                    res,
            ...
            Operator::LocalGet { local_index } => {
                let (type_value, pointer_value) =
                    self.locals[local_index as usize];
                let v =
                    err!(self.builder.build_load(type_value, pointer_value, ""));
                tbaa_label(
                    self.module,
                    self.intrinsics,
            ...
            Operator::GlobalGet { global_index } => {
                ...
                match self.ctx.global(
                    ...
                    GlobalCache::Mut {
                        ...
                        let value = err!(self.builder.build_load(
                            *value_type,
                            *ptr_to_value,
                            ""
                        ));
                ...
                match self.ctx.global(
                    ...
                    GlobalCache::Mut { ptr_to_value, .. } => {
                        ...
                        let value =
                            self.apply_pending_canonicalization(value, info)?;
                        let store =
                            err!(self.builder.build_store(ptr_to_value, value));
                        tbaa_label(
                            self.module,
                            self.intrinsics,
                            format!("global {}", global_index.as_u32()),
            ...
            Operator::TypedSelect { .. } | Operator::Select => {
                ...
                let (v1, i1, v2, i2) = if i1.has_pending_f32_nan()
                    != i2.has_pending_f32_nan()
                ...
                let res =
                    err!(self.builder.build_select(cond_value, v1, v2, ""));
                let info = {
            ...
            Operator::Call { function_index } => {
                ...
                let FunctionCache {
                    ...
                    vmctx: callee_vmctx,
                    attrs,
                } = if let Some(local_func_index) =
                    self.wasm_module.local_func_index(func_index)
                {
                ...
                */
                let params =
                    self.state.popn_save_extra(func_type.params().len())?;
            ...
            Operator::CallIndirect {
                ...
                let func_type = &self.wasm_module.signatures[sigindex];
                let expected_dynamic_sigindex = self.ctx.dynamic_sigindex(
                    sigindex,
                    self.intrinsics,
                    self.module,
                )?;
                ...
                let in_bounds_continue_block = self.context.append_basic_block(
                    self.function,
                    "in_bounds_continue_block",
                );
                ...
                {
                    let funcref_not_null = err!(self.builder.build_is_not_null(
                        anyfunc_struct_ptr,
                        "null funcref check"
                    ));

                    let funcref_continue_deref_block =
                        self.context.append_basic_block(
                            self.function,
                            "funcref_continue deref_block",
                        );

                    let funcref_is_null_block = self.context.append_basic_block(
                        self.function,
                ...
                let elem_initialized =
                    err!(self.builder.build_is_not_null(func_ptr, ""));
                ...
                let initialized_and_sigindices_match = err!(self
                    .builder
                ...
                let sigindices_notequal_block = self.context.append_basic_block(
                    self.function,
            ...
            | Operator::I16x8ExtAddPairwiseI8x16U => {
                let extend_op = match op {
                    Operator::I16x8ExtAddPairwiseI8x16S => |s: &Self, v| {
                        s.builder.build_int_s_extend(
                            v,
                            s.intrinsics.i16x8_ty,
                            "",
                        )
                    },
                    Operator::I16x8ExtAddPairwiseI8x16U => |s: &Self, v| {
                        s.builder.build_int_z_extend(
                            v,
                            s.intrinsics.i16x8_ty,
                            "",
                        )
            ...
            | Operator::I32x4ExtAddPairwiseI16x8U => {
                let extend_op = match op {
                    Operator::I32x4ExtAddPairwiseI16x8S => |s: &Self, v| {
                        s.builder.build_int_s_extend(
                            v,
                            s.intrinsics.i32x4_ty,
                            "",
                        )
                    },
                    Operator::I32x4ExtAddPairwiseI16x8U => |s: &Self, v| {
                        s.builder.build_int_z_extend(
                            v,
                            s.intrinsics.i32x4_ty,
                            "",
            ...
            Operator::I16x8Q15MulrSatS => {
                ...
                let max_value =
                    self.intrinsics.i16_ty.const_int(i16::MAX as u64, false);
                let max_values = VectorType::const_vector(&[max_value; 8]);

                let v1 = err!(self.builder.build_int_s_extend(
                    v1,
                    self.intrinsics.i32x8_ty,
                    ""
                ));
                let v2 = err!(self.builder.build_int_s_extend(
                    v2,
                    self.intrinsics.i32x8_ty,
                    ""
                ));
                ...
                let res = err!(self
                    .builder
                    .build_right_shift(res, fifteens, true, ""));
                let saturate_up = {
                ...
                let res = err!(self.builder.build_int_truncate(
                    res,
                    self.intrinsics.i16x8_ty,
                    ""
                ));
                ...
                let extend_op = match op {
                    ...
                    | Operator::I16x8ExtMulHighI8x16S => {
                        |s: &Self, v| -> Result<VectorValue, CompileError> {
                            err_nt!(s.builder.build_int_s_extend(
                                v,
                                s.intrinsics.i16x8_ty,
                                ""
                            ))
                    ...
                    | Operator::I16x8ExtMulHighI8x16U => {
                        |s: &Self, v| -> Result<VectorValue, CompileError> {
                            err_nt!(s.builder.build_int_z_extend(
                                v,
                                s.intrinsics.i16x8_ty,
                                ""
                            ))
            ...
            | Operator::I32x4ExtMulHighI16x8U => {
                let extend_op = match op {
                    Operator::I32x4ExtMulLowI16x8S
                    | Operator::I32x4ExtMulHighI16x8S => |s: &Self, v| {
                        s.builder.build_int_s_extend(
                            v,
                            s.intrinsics.i32x4_ty,
                            "",
                    ...
                    Operator::I32x4ExtMulLowI16x8U
                    | Operator::I32x4ExtMulHighI16x8U => |s: &Self, v| {
                        s.builder.build_int_z_extend(
                            v,
                            s.intrinsics.i32x4_ty,
                            "",
            ...
            | Operator::I64x2ExtMulHighI32x4U => {
                let extend_op = match op {
                    Operator::I64x2ExtMulLowI32x4S
                    | Operator::I64x2ExtMulHighI32x4S => |s: &Self, v| {
                        s.builder.build_int_s_extend(
                            v,
                            s.intrinsics.i64x2_ty,
                            "",
                        )
                    },
                    Operator::I64x2ExtMulLowI32x4U
                    | Operator::I64x2ExtMulHighI32x4U => |s: &Self, v| {
                        s.builder.build_int_z_extend(
                            v,
                            s.intrinsics.i64x2_ty,
                            "",
            ...
            Operator::I32x4DotI16x8S => {
                ...
                let low_product =
                    err!(self.builder.build_int_mul(v1_low, v2_low, ""));
                let high_product =
                    err!(self.builder.build_int_mul(v1_high, v2_high, ""));
            ...
            Operator::I32RemS | Operator::I64RemS => {
                ...
                let (min_value, neg_one_value) = if int_type
                    == self.intrinsics.i32_ty
                {
                    let min_value = int_type.const_int(i32::MIN as u64, false);
                    let neg_one_value =
                        int_type.const_int(-1i32 as u32 as u64, false);
                    (min_value, neg_one_value)
                } else if int_type == self.intrinsics.i64_ty {
                    let min_value = int_type.const_int(i64::MIN as u64, false);
                    let neg_one_value = int_type.const_int(-1i64 as u64, false);
                ...
                let v1 = err!(self.builder.build_select(
                    will_overflow,
                    int_type.const_zero(),
                    v1,
                    ""
            ...
            Operator::V128Bitselect => {
                let ((v1, i1), (v2, i2), (cond, cond_info)) =
                    self.state.pop3_extra()?;
                let v1 = self.apply_pending_canonicalization(v1, i1)?;
                let v2 = self.apply_pending_canonicalization(v2, i2)?;
                let cond =
                    self.apply_pending_canonicalization(cond, cond_info)?;
                let v1 = err!(self.builder.build_bit_cast(
                    v1,
                    self.intrinsics.i1x128_ty,
                    ""
                ))
                .into_vector_value();
                let v2 = err!(self.builder.build_bit_cast(
                    v2,
                    self.intrinsics.i1x128_ty,
                    ""
                ))
                .into_vector_value();
                let cond = err!(self.builder.build_bit_cast(
                    cond,
                    self.intrinsics.i1x128_ty,
                    ""
                ))
            ...
            Operator::I64x2Shl => {
                ...
                let v2 = err!(self.builder.build_int_z_extend(
                    v2,
                    self.intrinsics.i64_ty,
                    ""
                ));
            ...
            Operator::I32ShrU => {
                ...
                let v2 = err!(self.builder.build_and(v2, mask, ""));
                let res =
                    err!(self.builder.build_right_shift(v1, v2, false, ""));
                self.state.push1(res);
            }
            Operator::I64ShrU => {
                let ((v1, i1), (v2, i2)) = self.state.pop2_extra()?;
            ...
            Operator::I8x16Abs => {
                ...
                let all_sign_bits =
                    err!(self.builder.build_right_shift(v, seven, true, ""));
                let xor = err!(self.builder.build_xor(v, all_sign_bits, ""));
                let res =
                    err!(self.builder.build_int_sub(xor, all_sign_bits, ""));
                let res = err!(self.builder.build_bit_cast(
                    res,
                    self.intrinsics.i128_ty,
                    ""
            ...
            Operator::I16x8Abs => {
                ...
                let all_sign_bits =
                    err!(self.builder.build_right_shift(v, fifteen, true, ""));
                let xor = err!(self.builder.build_xor(v, all_sign_bits, ""));
                let res =
                    err!(self.builder.build_int_sub(xor, all_sign_bits, ""));
                let res = err!(self.builder.build_bit_cast(
            ...
            Operator::I32x4Abs => {
                ...
                let all_sign_bits =
                    err!(self.builder.build_right_shift(v, thirtyone, true, ""));
                let xor = err!(self.builder.build_xor(v, all_sign_bits, ""));
                let res =
                    err!(self.builder.build_int_sub(xor, all_sign_bits, ""));
                let res = err!(self.builder.build_bit_cast(
            ...
            Operator::I64x2Abs => {
                ...
                let all_sign_bits = err!(self
                    .builder
                    .build_right_shift(v, sixtythree, true, ""));
                let xor = err!(self.builder.build_xor(v, all_sign_bits, ""));
                let res =
                    err!(self.builder.build_int_sub(xor, all_sign_bits, ""));
                let res = err!(self.builder.build_bit_cast(
            ...
            Operator::I8x16AvgrU => {
                ...
                let res =
                    err!(self.builder.build_right_shift(res, one, false, ""));
                let res = err!(self.builder.build_int_truncate(
                    res,
                    self.intrinsics.i8x16_ty,
                    ""
            ...
            Operator::F32x4Sqrt => {
                ...
                .left()
                .unwrap();
                let bits = err!(self.builder.build_bit_cast(
                    res,
                    self.intrinsics.i128_ty,
                    "bits"
                ));
            ...
            Operator::F32x4PMin => {
                ...
                let cmp = err!(self.builder.build_float_compare(
                    FloatPredicate::OLT,
                    v2,
                    v1,
                    ""
            ...
            Operator::F32Copysign => {
                let ((mag, mag_info), (sgn, sgn_info)) =
                    self.state.pop2_extra()?;
            ...
            Operator::I8x16Eq => {
                ...
                let res = err!(self.builder.build_int_s_extend(
                    res,
                    self.intrinsics.i8x16_ty,
                    ""
                ));
            ...
            Operator::F32Eq | Operator::F64Eq => {
                let (v1, v2) = self.state.pop2()?;
                let (v1, v2) = (v1.into_float_value(), v2.into_float_value());
                let cond = err!(self.builder.build_float_compare(
                    FloatPredicate::OEQ,
                    v1,
                    v2,
                    ""
            ...
            Operator::F32x4Eq => {
                ...
                let (v1, _) = self.v128_into_f32x4(v1, i1)?;
                let (v2, _) = self.v128_into_f32x4(v2, i2)?;
                let res = err!(self.builder.build_float_compare(
                    FloatPredicate::OEQ,
                    v1,
                    v2,
                    ""
            ...
            | Operator::I64x2ExtendHighI32x4S => {
                let extend = match op {
                    Operator::I64x2ExtendLowI32x4U
                    | Operator::I64x2ExtendHighI32x4U => |s: &Self, v| {
                        s.builder.build_int_z_extend(
                            v,
                            s.intrinsics.i64x2_ty,
                            "",
                    ...
                    Operator::I64x2ExtendLowI32x4S
                    | Operator::I64x2ExtendHighI32x4S => |s: &Self, v| {
                        s.builder.build_int_s_extend(
                            v,
                            s.intrinsics.i64x2_ty,
                            "",
            ...
            Operator::I8x16NarrowI16x8S => {
                ...
                let apply_min_clamp_v1 = err!(self.builder.build_int_compare(
                    IntPredicate::SLT,
                    v1,
                    min,
                    ""
                ));
                let apply_max_clamp_v1 = err!(self.builder.build_int_compare(
                    IntPredicate::SGT,
                    v1,
                    max,
                    ""
                ));
                let apply_min_clamp_v2 = err!(self.builder.build_int_compare(
                    IntPredicate::SLT,
                    v2,
                    min,
                    ""
                ));
                let apply_max_clamp_v2 = err!(self.builder.build_int_compare(
                    IntPredicate::SGT,
                    v2,
                    max,
                    ""
                ...
                let v2 = err!(self.builder.build_select(
                    apply_min_clamp_v2,
                    min,
                    v2,
                    ""
            ...
            | Operator::F64x2ConvertLowI32x4U => {
                let extend = match op {
                    Operator::F64x2ConvertLowI32x4U => |s: &Self, v| {
                        s.builder.build_int_z_extend(
                            v,
                            s.intrinsics.i64x2_ty,
                            "",
                    ...
                    Operator::F64x2ConvertLowI32x4S => |s: &Self, v| {
                        s.builder.build_int_s_extend(
                            v,
                            s.intrinsics.i64x2_ty,
                            "",
            ...
            Operator::F64x2PromoteLowF32x4 => {
                ...
                let res = err!(self.builder.build_float_ext(
                    low,
                    self.intrinsics.f64x2_ty,
                    ""
                ));
            ...
            Operator::I32ReinterpretF32 => {
                let (v, i) = self.state.pop1_extra()?;
                let v = self.apply_pending_canonicalization(v, i)?;
                let ret = err!(self.builder.build_bit_cast(
                    v,
                    self.intrinsics.i32_ty,
                    ""
                ));
            ...
            Operator::V128Load8Lane { ref memarg, lane } => {
                ...
                let res =
                    err!(self.builder.build_insert_element(v, element, idx, ""));
                let res = err!(self.builder.build_bit_cast(
                    res,
                    self.intrinsics.i128_ty,
                    ""
            ...
            Operator::I32Store { ref memarg } => {
                ...
                let store =
                    err!(self.builder.build_store(effective_address, value));
            ...
            Operator::V128Store8Lane { ref memarg, lane } => {
                ...
                let store =
                    err!(self.builder.build_store(effective_address, val));
                self.annotate_user_memaccess(memory_index, memarg, 1, store)?;
            }
            ...
            | Operator::I64Store8 { ref memarg } => {
                ...
                let store = err!(self
                    .builder
                    .build_store(effective_address, narrow_value));
                self.annotate_user_memaccess(memory_index, memarg, 1, store)?;
            }
            ...
            Operator::I8x16Neg => {
                let (v, i) = self.state.pop1_extra()?;
                let (v, _) = self.v128_into_i8x16(v, i)?;
                let res = err!(self.builder.build_int_sub(
            ...
            Operator::V128Not => {
                let (v, i) = self.state.pop1_extra()?;
                let v =
                    self.apply_pending_canonicalization(v, i)?.into_int_value();
            ...
            | Operator::I64x2AllTrue => {
                ...
                let v =
                    self.apply_pending_canonicalization(v, i)?.into_int_value();
                let lane_int_ty =
                    self.context.custom_width_int_type(vec_ty.get_size());
                let vec = err!(self.builder.build_bit_cast(v, vec_ty, "vec"))
                    .into_vector_value();
                ...
                let cmask = err!(self.builder.build_bit_cast(
                    mask,
                    lane_int_ty,
                    "cmask"
                ))
            ...
            Operator::I8x16ReplaceLane { lane } => {
                ...
                let (v1, _) = self.v128_into_i8x16(v1, i1)?;
                let v2 = v2.into_int_value();
                let v2 = err!(self.builder.build_int_cast(
                    v2,
                    self.intrinsics.i8_ty,
                    ""
                ));
                let idx = self.intrinsics.i32_ty.const_int(lane.into(), false);
                let res =
                    err!(self.builder.build_insert_element(v1, v2, idx, ""));
                let res = err!(self.builder.build_bit_cast(
                    res,
                    self.intrinsics.i128_ty,
                    ""
            ...
            Operator::I32x4ReplaceLane { lane } => {
                ...
                self.state.push1_extra(
                    res,
                    ((i1 & i2)? & ExtraInfo::arithmetic_f32())?,
                );
            }
            Operator::I64x2ReplaceLane { lane } => {
                ...
                self.state.push1_extra(
                    res,
                    ((i1 & i2)? & ExtraInfo::arithmetic_f64())?,
                );
            }
            ...
            Operator::I8x16Swizzle => {
                ...
                for i in 0..16 {
                    ...
                    let replace_with_zero =
                        err!(self.builder.build_extract_element(
                            idx_out_of_range,
                            self.intrinsics.i32_ty.const_int(i, false),
                            "replace_with_zero",
                        ))
            ...
            Operator::I8x16Shuffle { lanes } => {
                ...
                let mask = VectorType::const_vector(
                    lanes
                        .iter()
                        .map(|l| {
                            self.intrinsics.i32_ty.const_int((*l).into(), false)
                        })
                        .collect::<Vec<IntValue>>()
                        .as_slice(),
                );
                let res =
                    err!(self.builder.build_shuffle_vector(v1, v2, mask, ""));
                let res = err!(self.builder.build_bit_cast(
                    res,
                    self.intrinsics.i128_ty,
                    ""
            ...
            Operator::I32AtomicRmw8AddU { ref memarg } => {
                ...
                let old = err!(self.builder.build_int_z_extend(
                    old,
                    self.intrinsics.i32_ty,
                    ""
                ));
            ...
            Operator::MemoryGrow { mem } => {
                ...
                let grow_fn_ptr =
                    self.ctx.memory_grow(memory_index, self.intrinsics)?;
            ...
            Operator::MemorySize { mem } => {
                let memory_index = MemoryIndex::from_u32(mem);
                let size_fn_ptr =
                    self.ctx.memory_size(memory_index, self.intrinsics)?;
            ...
            Operator::MemoryInit { data_index, mem } => {
                ...
                let segment =
                    self.intrinsics.i32_ty.const_int(data_index.into(), false);
                err!(self.builder.build_call(
            ...
            Operator::MemoryCopy { dst_mem, src_mem } => {
                ...
                let src_index =
                    self.intrinsics.i32_ty.const_int(src.into(), false);
                err!(self.builder.build_call(
                    memory_copy,
                    &[
                        vmctx.as_basic_value_enum().into(),
            ...
            Operator::MemoryFill { mem } => {
                ...
                let mem_index =
                    self.intrinsics.i32_ty.const_int(mem.into(), false);
                err!(self.builder.build_call(
                    memory_fill,
                    &[
                        vmctx.as_basic_value_enum().into(),
            ...
            Operator::TableGet { table } => {
                let table_index =
                    self.intrinsics.i32_ty.const_int(table.into(), false);
            ...
            Operator::TableCopy {
                ...
                let dst_table =
                    self.intrinsics.i32_ty.const_int(dst_table as u64, false);
                let src_table =
                    self.intrinsics.i32_ty.const_int(src_table as u64, false);
                err!(self.builder.build_call(
            ...
            Operator::TableInit { elem_index, table } => {
                let (dst, src, len) = self.state.pop3()?;
                let segment =
                    self.intrinsics.i32_ty.const_int(elem_index as u64, false);
                let table =
                    self.intrinsics.i32_ty.const_int(table as u64, false);
                err!(self.builder.build_call(
            ...
            Operator::TableFill { table } => {
                ...
                let elem = err!(self.builder.build_bit_cast(
                    elem,
                    self.intrinsics.ptr_ty,
                    ""
                ));
            ...
            Operator::TableGrow { table } => {
                ...
                let table_index =
                    self.intrinsics.i32_ty.const_int(table_index as u64, false);
            ...
            Operator::MemoryAtomicWait32 { memarg } => {
                ...
                let wait32_fn_ptr =
                    self.ctx.memory_wait32(memory_index, self.intrinsics)?;
            ...
            Operator::MemoryAtomicWait64 { memarg } => {
                ...
                let (dst, val, timeout) = self.state.pop3()?;
                let wait64_fn_ptr =
                    self.ctx.memory_wait64(memory_index, self.intrinsics)?;
                let ret = err!(self.builder.build_indirect_call(
            ...
            Operator::MemoryAtomicNotify { memarg } => {
                ...
                let notify_fn_ptr =
                    self.ctx.memory_notify(memory_index, self.intrinsics)?;
