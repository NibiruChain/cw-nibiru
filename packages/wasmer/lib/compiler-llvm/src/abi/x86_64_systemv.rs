...
impl Abi for X86_64SystemV {
    ...
    fn rets_from_call<'ctx>(
        ...
        let extract_f32x2 = |value: VectorValue<'ctx>| -> Result<
            (FloatValue<'ctx>, FloatValue<'ctx>),
            CompileError,
        > {
            assert!(value.get_type() == f32x2_ty.into_vector_type());
            let ret0 = err!(builder.build_extract_element(
                value,
                intrinsics.i32_ty.const_int(0, false),
                ""
            ))
            .into_float_value();
            let ret1 = err!(builder.build_extract_element(
                value,
        ...
        if let Some(basic_value) = call_site.try_as_basic_value().left() {
            if func_sig.results().len() > 1 {
                ...
                if basic_value.get_type() == f32x2_ty {
                    assert!(func_sig.results().len() == 2);
                    let (ret0, ret1) =
                        extract_f32x2(basic_value.into_vector_value())?;
                    return Ok(vec![ret0.into(), ret1.into()]);
                }
                let struct_value = basic_value.into_struct_value();
                let rets = (0..struct_value.get_type().count_fields())
                    .map(|i| {
                        builder.build_extract_value(struct_value, i, "").unwrap()
                    })
                    .collect::<Vec<_>>();
                ...
                let ret = match func_sig_returns_bitwidths.as_slice() {
                                ...
                                .as_basic_type_enum() =>
                    {
                        ...
                        let (rets0, rets1) =
                            extract_f32x2(rets[0].into_vector_value())?;
                                ...
                                .as_basic_type_enum() =>
                    {
                        assert!(func_sig.results().len() == 3);
                        let (rets1, rets2) =
                            extract_f32x2(rets[1].into_vector_value())?;
                        vec![rets[0], rets1.into(), rets2.into()]
                    }
                    [64, 32, 32] => {
                        assert!(func_sig.results().len() == 3);
                        let (rets1, rets2) =
                            split_i64(rets[1].into_int_value())?;
    ...
    fn pack_values_for_register_return<'ctx>(
        ...
        let pack_f32s = |first: BasicValueEnum<'ctx>,
         ...
         -> Result<BasicValueEnum<'ctx>, CompileError> {
            ...
            let (first, second) =
                (first.into_float_value(), second.into_float_value());
        ...
        let build_struct =
            |ty: StructType<'ctx>, values: &[BasicValueEnum<'ctx>]| {
        ...
        Ok(match *values {
            ...
            [v1, v2, v3] if is_64(v1) && is_f32(v2) && is_f32(v3) => {
                build_struct(
                    func_type.get_return_type().unwrap().into_struct_type(),
                    &[v1, pack_f32s(v2, v3)?],
                )
                ...
                if is_32(v1) && is_32(v2) && is_32(v3) && is_32(v4) =>
            {
                let v1v2_pack = if is_f32(v1) && is_f32(v2) {
                    ...
                    let v1 =
                        err!(builder.build_bit_cast(v1, intrinsics.i32_ty, ""));
                    let v2 =
                        err!(builder.build_bit_cast(v2, intrinsics.i32_ty, ""));
                    pack_i32s(v1, v2)?
                };
                let v3v4_pack = if is_f32(v3) && is_f32(v4) {
                    ...
                    let v3 =
                        err!(builder.build_bit_cast(v3, intrinsics.i32_ty, ""));
                    let v4 =
                        err!(builder.build_bit_cast(v4, intrinsics.i32_ty, ""));
                    pack_i32s(v3, v4)?
                };
                build_struct(
                    func_type.get_return_type().unwrap().into_struct_type(),
